...
- **纯 Shell 会话**：模型下拉框内置 CMD / PowerShell / 系统默认 shell，一键开远程 shell（不跑 Claude）
- **Web 文件管理器（PC）**：全盘浏览，在线编辑/预览/上传/下载/重命名/删除，支持 Markdown 与 HTML 渲染预览，复用同一 Bearer 鉴权
- **访问控制**：简易令牌门禁，防止未授权访问
...
### Web 文件管理器（PC）
- 在主页的 **📁 文件** 入口打开，独立 view tab；驱动器选择 + 路径栏 + 列表导航，支持全盘浏览
- 文本文件可在线编辑（扩展名白名单 + NUL 字节嗅探，1 MiB 上限）；Markdown 与 HTML 一键切换"渲染预览/源码编辑"
- 图片预览，列表与预览面板可拖拽分割条调整宽度，支持单文件上传、新建文件夹、重命名、删除
- 所有端点复用与其他模块同一套 Bearer 鉴权，即便外网（frp）暴露下也只有持令牌者可访问
- 路径安全：入参一律 `Path.resolve()` 成绝对路径；mkdir/rename 的新名必须是单一路径分量（含分隔符或 `..` 直接拒绝）；delete 拒绝盘符根目录兜底防误删

### 纯 Shell 会话（不跑 Claude）
- 模型下拉框新增 **CMD / PowerShell / 默认 Shell** 项；选中后直接启动纯 shell，而不是 Claude
- 多页签体验一致：shell 跑在 web 终端里，WS 断开自动重连，独立的会话记录
- 适合只想远程开个 shell、又不想浪费一次 Claude 对话的场景
